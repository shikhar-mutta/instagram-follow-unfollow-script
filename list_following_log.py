#!/usr/bin/env python3
"""
Fetch every account the logged-in user follows and save them to a log file
in the same tab-separated format used by refollow_cycle.py:
...
Usage:
    python list_following_log.py [credentials_file]
...
LOG_FILE = Path("following_log.txt")
...
def main() -> None:
    ...
    for user in following.values():
        ...
        lines.append(f"{timestamp}\t{account}\t{ACTION}\t{user.username}\t{url}")

    header = f"Total following: {len(lines)}"
    body = "\n".join([header, *lines])
    LOG_FILE.write_text(body + "\n", encoding="utf-8")

    print(f"Saved {len(lines)} following to {LOG_FILE.resolve()}")
